...
esp_err_t Ota::CheckVersion() {
    ...
    if (cJSON_IsObject(server_time)) {
        ...
        if (cJSON_IsNumber(timestamp)) {
            // timestamp is a Unix epoch in milliseconds.  settimeofday() also
            // expects UTC epoch time; display timezone is applied by localtime_r().
            // Adding timezone_offset here shifts local time a second time.
            struct timeval tv;
