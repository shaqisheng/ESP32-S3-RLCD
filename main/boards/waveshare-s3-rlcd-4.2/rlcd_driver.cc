...
constexpr int kSpiChunkSize = 1024;
      ...
      width_(width), height_(height)
{
    ...
    // A full monochrome frame is 15 KB. Keep the DMA transaction small so AFE,
    // MQTT and the display do not all require one large contiguous SRAM block.
    buscfg.max_transfer_sz = kSpiChunkSize;
    ...
    // One queued transfer also guarantees the previous PSRAM-backed chunk has
    // completed before the next one needs a DMA bounce buffer.
...
void RlcdDriver::RLCD_Sendbuffera(uint8_t *Data, int len) {
    for (int offset = 0; offset < len; offset += kSpiChunkSize) {
        const int chunk_len = std::min(kSpiChunkSize, len - offset);
        esp_err_t result = ESP_FAIL;
        for (int attempt = 0; attempt < 3; ++attempt) {
            result = esp_lcd_panel_io_tx_color(io_handle_, -1, Data + offset, chunk_len);
            if (result == ESP_OK) break;
            if (result != ESP_ERR_NO_MEM) break;
            ESP_LOGW(TAG, "屏幕 SPI 暂时无可用内存，第 %d 次重试", attempt + 1);
            vTaskDelay(pdMS_TO_TICKS(20));
        }
        if (result != ESP_OK) {
            ESP_LOGE(TAG, "屏幕 SPI 分片发送失败: %s", esp_err_to_name(result));
            return;
        }
    }
}
