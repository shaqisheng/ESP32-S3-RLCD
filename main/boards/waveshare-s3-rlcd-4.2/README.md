...
### 国内城市自动天气（当前默认）
- **数据来源：** 和风天气实时天气与七日预报接口
- **配置方式：** 登录局域网后台，选择省份和城市，并一次性保存和风 API Key（Key 不会回显）
- **设备侧行为：** 连续空闲且联网时自动刷新，刷新间隔 30 分钟
- **显示内容：** 城市、当前天气、体感、湿度、风速和七日预报

`self.weather.update` 仍作为外部天气数据的临时回写兼容入口，但下一次设备自动刷新会恢复所选城市的数据。
...
### 天气不更新
1. 在后台“城市天气”中确认省份和城市并保存
2. 确认设备联网且处于空闲状态
3. 查看串口日志是否出现 `和风天气更新成功`、`未配置和风 API Key` 或具体网络错误
...
- 国内城市自动天气与 MCP 兼容回写（`self.weather.update`）
- 备忘录基础能力（新增 / 列表 / 完成 / 清空）
