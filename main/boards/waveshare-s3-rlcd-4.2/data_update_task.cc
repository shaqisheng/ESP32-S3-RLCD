...
void CustomLcdDisplay::DataUpdateTask(void *arg) {
    ...
    bool boot_prefetch_observed = false;
    ...
    bool last_weather_request_ok = false;
    uint32_t last_holiday_attempt_ms = 0;
    int holiday_synced_year = CalendarManager::GetInstance().GetCachedYear();
    ...
    while (1) {
        ...
        if (!boot_prefetch_observed && rlcd::BootDataPrefetchCompleted()) {
            boot_prefetch_observed = true;
            time_synced = rlcd::BootTimePrefetched();
            if (time_synced) {
                ntp_retry_count = 0;
                ntp_last_sync_ms = now_ms;
                time(&self->last_valid_epoch_);
            } else {
                // The boot attempt already consumed its bounded retry window.
                // Keep RTC time and retry later through the normal idle path.
                ntp_retry_count = 1;
            }
            last_weather_request_ok = rlcd::BootWeatherPrefetched();
            last_weather_update_ms = now_ms;
            self->last_min_ = -1;
            ESP_LOGI(TAG, "已接收开机预取结果: 时间=%s, 天气=%s",
                     time_synced ? "网络同步" : "RTC回退",
                     last_weather_request_ok ? "已更新" : "缓存回退");
        }
        const bool in_audio_session = (ds == kDeviceStateConnecting ||
                                       ds == kDeviceStateListening ||
                                       ds == kDeviceStateSpeaking);
        // 联网能力必须以实际 Wi-Fi 状态为准，不能用小智服务状态代替。
        // 这样 AI 服务异常不会阻塞天气，而 Wi-Fi 断开时也不会无效发起 HTTPS 请求。
        const bool wifi_connected = WifiManager::GetInstance().IsConnected();
        const bool can_run_background_requests =
            wifi_connected && rlcd::BootDataPrefetchCompleted() && !in_audio_session &&
            ds != kDeviceStateStarting &&
            ds != kDeviceStateWifiConfiguring &&
            ds != kDeviceStateUpgrading &&
            ds != kDeviceStateAudioTesting;

        // ===== 连续 idle 计时 =====
        // NTP / 天气等网络请求必须在"连续 idle 超过 N 秒"后才发，
        // 防止刚开机或刚结束对话就发 HTTPS 请求，和下一次唤醒撞车导致首句卡顿
        const uint32_t IDLE_GUARD_MS = 30000;  // 启动及对话结束后留足 MQTT/TLS 内存恢复时间
        if (can_run_background_requests) {
            if (idle_since_ms == 0) {
        ...
        bool idle_long_enough =
            idle_since_ms > 0 &&
            now_ms - idle_since_ms >= IDLE_GUARD_MS &&
            idle_generation == rlcd::BackgroundNetworkGeneration();
        if (idle_since_ms > 0 && idle_generation != rlcd::BackgroundNetworkGeneration()) {
            idle_since_ms = now_ms;
            idle_generation = rlcd::BackgroundNetworkGeneration();
        }
        ...
        if (can_run_background_requests && idle_long_enough) {
            ...
            if (weather_requested || last_weather_update_ms == 0 ||
                now_ms - last_weather_update_ms >= due_interval_ms) {
                last_weather_request_ok = weather_manager.update();
                last_weather_update_ms = now_ms;
            }
            int year = timeinfo.tm_year + 1900;
            if (year >= 2024 && holiday_synced_year != year &&
                (last_holiday_attempt_ms == 0 || now_ms - last_holiday_attempt_ms >= 6 * 60 * 60 * 1000)) {
                last_holiday_attempt_ms = now_ms;
                if (CalendarManager::GetInstance().SyncYear(year)) holiday_synced_year = year;
            }
        }
        ...
        if (!self->showing_system_info_) {
            ...
            if (minute_changed) {
                ...
                if (self->date_detail_label_) {
                    static const char* weeks_cn[] = {"周日","周一","周二","周三","周四","周五","周六"};
                    char detail[96];
                    snprintf(detail, sizeof(detail), "%d年%d月%d日 %s\n农历%s",
                             timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday,
                             weeks_cn[timeinfo.tm_wday],
                             CalendarManager::LunarFullText(timeinfo.tm_year + 1900, timeinfo.tm_mon + 1, timeinfo.tm_mday).c_str());
                    lv_label_set_text(self->date_detail_label_, detail);
                }
        ...
        // 额度页快照变更与 10 秒子页轮播（网络刷新在独立低优先级任务中）
        self->TickQuotaPage();
