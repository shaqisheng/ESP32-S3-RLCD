...
private:
    ...
    bool state_listener_registered_ = false;
    ...
    void InitializeButtons() { 
        ...
        user_button_.OnClick([this]() {
            ...
            ESP_LOGI(TAG, "USER 按钮单击：切换已启用页面");
        });
    ...
    void RefreshAllData() {
        ...
        // 由低优先级更新任务在设备空闲时执行，避免手动刷新阻塞按键回调。
        ...
        // 强制刷新屏幕显示
        if (display_) {
            display_->SetChatMessage("system", "小智 · 待命\n时间已更新，天气正在获取");
        }
    ...
    void InitializeTools() {
        ...
        mcp_server.AddTool(
            "self.disp.switch",
            "Switch display page between overview, calendar, forecast, and AI quota.\n"
            "Use when user says: '打开综合页', '查看日历', '七日天气', '查看额度', '切换屏幕'.\n"
            "Args:\n"
            "  `mode`: 'toggle' | 'overview' | 'calendar' | 'forecast' | 'quota' (default: 'toggle')",
            PropertyList({
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                if (mode == "toggle") {
                    ...
                    display_->SwitchToCalendarPage();
                } else if (mode == "forecast") {
                    display_->SwitchToForecastPage();
                } else if (mode == "quota") {
                    display_->SwitchToQuotaPage();
                } else {
                    return std::string("参数 mode 无效，请使用 toggle/overview/calendar/forecast/quota");
                }
...
public:
    ...
    void StartNetwork() override {
        // httpd and quota fetching use lwIP.  Start them only after WifiBoard has
        // initialized the TCP/IP stack; doing this in the constructor is too early.
        WifiBoard::StartNetwork();
        if (!state_listener_registered_) {
            auto& app = Application::GetInstance();
            app.AddStateChangeListener([](DeviceState, DeviceState new_state) {
            ...
            state_listener_registered_ = true;
        }
        QuotaManager::GetInstance().Start();
        AdminServer::GetInstance().Start();
    }

    void PrepareForActivation() override {
        ESP_LOGI(TAG, "[BOOT 1/3] Wi-Fi ready; synchronizing dashboard time");
        const bool time_ok = SensorManager::getInstance().syncNtpTime();

        ESP_LOGI(TAG, "[BOOT 2/3] Fetching dashboard weather before Xiaozhi registration");
        // This boot fetch fulfills the manager's initial refresh request. Consume
        // it first so the periodic task does not repeat the same HTTPS request
        // after its 30-second idle guard.
        WeatherManager::getInstance().TakeRefreshRequest();
        const bool weather_ok = WeatherManager::getInstance().update();
        rlcd::MarkBootDataPrefetched(time_ok, weather_ok);

        ESP_LOGI(TAG, "[BOOT 3/3] Dashboard prefetch complete (time=%s, weather=%s)",
                 time_ok ? "ok" : "fallback",
                 weather_ok ? "ok" : "fallback");
    }
