...
<section class="panel"><h2>AI 页显示</h2><div class="manage-grid" style="grid-template-columns:repeat(3,1fr)"><div class="field"><label>每屏卡片数<select id="quotaCardsPerPage" onchange="renderForcePageOptions()"><option value="1">1 张</option><option value="2">2 张</option><option value="3">3 张</option><option value="4" selected>4 张</option></select></label></div><div class="field"><label>自动翻页（秒，0=不翻）<input id="quotaAutoAdvance" type="number" min="0" max="120" step="1"></label></div><div class="field"><label>固定页码<select id="quotaForcePage"></select></label></div></div><div class="toolbar" style="justify-content:flex-start"><button onclick="saveQuotaDisplay()">保存显示配置</button></div><p class="hint">固定页码可选项由当前账号数 + 每屏卡片数计算；0=自动翻页。固定页码优先于自动翻页。</p></section>
