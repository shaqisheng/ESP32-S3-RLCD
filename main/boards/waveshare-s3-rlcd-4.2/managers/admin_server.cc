...
<div class="field"><label class="proxy-toggle"><input type="checkbox" id="powerSaveRestDay" onchange="savePowerSave()"> 法定节假日/周末/调休日全天启用省电（无视夜间时段）</label></div>
...
esp_err_t AdminServer::DisplaySwitchHandler(httpd_req_t* req) {
    ...
    if (mode != "toggle" && mode != "overview" && mode != "weather" && mode != "calendar" &&
        mode != "forecast" && mode != "quota" && mode != "todo" && mode != "info") {
