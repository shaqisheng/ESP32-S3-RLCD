...
// 省电模式
async function loadPowerSave(){try{var d=await api("/api/power-save");el("powerSaveStatus").textContent=d.active?"已启用":"已关闭";el("powerSaveReason").textContent=d.active?(d.reason||"--"):"--";el("powerSaveThreshold").value=d.battery_threshold||20;el("powerSaveNightStart").value=d.night_start_hour||23;el("powerSaveNightEnd").value=d.night_end_hour||7;el("powerSaveNightEnabled").checked=d.night_enabled||false;el("powerSaveToggleBtn").textContent=d.manual_override?"关闭省电模式":"开启省电模式"}catch(e){}}
async function togglePowerSave(btn){var cur=btn.textContent==="关闭省电模式";try{await api("/api/power-save",{method:"PUT",body:JSON.stringify({manual_override:!cur,battery_threshold:Number(el("powerSaveThreshold").value),night_start_hour:Number(el("powerSaveNightStart").value),night_end_hour:Number(el("powerSaveNightEnd").value),night_enabled:el("powerSaveNightEnabled").checked})});toast(cur?"已关闭省电模式":"已开启省电模式");loadPowerSave()}catch(e){toast(e.message,true)}}
