...
<section class="panel"><h2>AI 页显示</h2><div class="manage-grid" style="grid-template-columns:repeat(3,1fr)"><div class="field"><label>每屏卡片数<select id="quotaCardsPerPage" onchange="renderForcePageOptions()"><option value="1">1 张</option><option value="2">2 张</option><option value="3">3 张</option><option value="4" selected>4 张</option></select></label></div><div class="field"><label>固定页码<select id="quotaForcePage" onchange="toggleAutoAdvanceVisibility()"></select></label></div><div class="field" id="autoAdvanceField"><label>自动翻页（秒，0=不翻）<input id="quotaAutoAdvance" type="number" min="0" max="120" step="1"></label></div></div><div class="toolbar" style="justify-content:flex-start"><button onclick="saveQuotaDisplay()">保存显示配置</button></div><p class="hint">固定页码可选项由当前账号数 + 每屏卡片数计算；选'自动翻页'时才需配置翻页间隔。</p></section>
...
<div class="toolbar" style="margin:14px 0;justify-content:flex-start"><button class="primary" id="powerSaveToggleBtn" onclick="togglePowerSave(this)">开启省电模式</button></div>
<div class="manage-grid" style="grid-template-columns:repeat(3,1fr)"><div class="field"><label>电池阈值（%）<input id="powerSaveThreshold" type="number" min="5" max="50" step="1"></label></div><div class="field"><label>夜间开始<select id="powerSaveNightStart" onchange="renderNightCrossDay()"></select></label></div><div class="field"><label>夜间结束<select id="powerSaveNightEnd" onchange="renderNightCrossDay()"></select></label></div></div>
<p class="hint" id="nightCrossDayHint" style="display:none;margin-top:4px">（跨天：今晚到明早）</p>
...
<div class="field"><label class="proxy-toggle"><input type="checkbox" id="powerSaveWorkdayOnly" onchange="savePowerSave()"> 仅工作日启用（法定节假日/周末/调休日不省电）</label></div>
...
<div class="modal-bg" id="wifiModal"><div class="modal"><h3>新增 Wi-Fi</h3><p class="hint">保存到设备，下次遇到该网络自动连接。</p><div class="field"><label>SSID<input id="wifiModalSsid" placeholder="网络名称"></label></div><div class="field"><label>密码<input id="wifiModalPassword" type="password" placeholder="Wi-Fi 密码"></label></div><div class="modal-actions"><button onclick="closeWifiModal()">取消</button><button class="primary" onclick="submitWifiModal()">保存</button></div></div></div>
...
function renderWifiSaved(list){if(!list.length){el("wifiSaved").innerHTML='<div class="empty">暂无已保存 Wi-Fi</div>';return}var h="";list.forEach(function(w){h+='<div class="todo-row" style="grid-template-columns:1fr auto;gap:12px"><b style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(w.ssid)+(w.is_current?' <span class="badge">当前</span>':"")+'</b><span style="white-space:nowrap"><button onclick="wifiSetDefault('+w.index+')">设默认</button> <button onclick="wifiConnect(\''+esc(w.ssid)+'\')">连接</button> <button class="danger" onclick="wifiDelete('+w.index+')">删除</button></span></div>'});el("wifiSaved").innerHTML=h}
...
// 根据账号数 + 每屏卡数计算固定页码可选项
function renderForcePageOptions(){var cards=Number(el("quotaCardsPerPage").value)||4;var total=items.length;var pages=Math.max(1,Math.ceil(total/cards));var cur=el("quotaForcePage").value;var h='<option value="0">自动翻页</option>';for(var i=1;i<=pages;i++){h+='<option value="'+i+'">第 '+i+' 页</option>'}el("quotaForcePage").innerHTML=h;if(cur&&Number(cur)<=pages)el("quotaForcePage").value=cur;toggleAutoAdvanceVisibility()}
// 固定页码='自动翻页'(0) 时显示翻页间隔输入；选具体页时隐藏
function toggleAutoAdvanceVisibility(){el("autoAdvanceField").style.display=el("quotaForcePage").value==="0"?"":"none"}
...
if(x.provider==="manual")h+=field(i,"manual_5h_remaining","5 小时剩余（0-100%）","number")+field(i,"manual_weekly_remaining","周剩余（0-100%）","number");
