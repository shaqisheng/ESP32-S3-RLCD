...
void Application::ActivationTask() {
    auto& board = Board::GetInstance();
    ESP_LOGI(TAG, "Preparing board data before Xiaozhi service registration");
    board.PrepareForActivation();
    ESP_LOGI(TAG, "Board data preparation finished; starting Xiaozhi activation");
...
void Application::SendMcpMessage(std::string payload) {
    ...
    Schedule([this, payload = std::move(payload)]() mutable {
        if (protocol_) {
            protocol_->SendMcpMessage(std::move(payload));
        }
    });
}
