...
namespace {
// RLCD 设备在音频与 TLS 活跃时内部 SRAM 余量很小，工具清单必须分页发送。
constexpr size_t kToolsListMaxPayloadSize = 2048;
constexpr size_t kMcpEnvelopeReserve = 512;
}
...
void McpServer::AddTool(McpTool* tool) {
    if (std::find(disabled_tools_.begin(), disabled_tools_.end(), tool->name()) != disabled_tools_.end()) {
        ESP_LOGI(TAG, "Skip disabled tool: %s", tool->name().c_str());
        delete tool;
        return;
    }
...
void McpServer::DisableTool(const std::string& name) {
    if (std::find(disabled_tools_.begin(), disabled_tools_.end(), name) == disabled_tools_.end()) {
        disabled_tools_.push_back(name);
    }

    auto it = std::remove_if(tools_.begin(), tools_.end(), [&name](McpTool* tool) {
...
void McpServer::ReplyResult(int id, std::string result) {
    const std::string prefix = "{\"jsonrpc\":\"2.0\",\"id\":" +
                               std::to_string(id) + ",\"result\":";
    result.reserve(result.size() + prefix.size() + 1);
    result.insert(0, prefix);
    result.push_back('}');
    Application::GetInstance().SendMcpMessage(std::move(result));
}
...
void McpServer::GetToolsList(int id, const std::string& cursor, bool list_user_only_tools) {
    ...
    // 预留值超过 CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL，确保主缓冲优先进入 PSRAM。
    json.reserve(kToolsListMaxPayloadSize + kMcpEnvelopeReserve);
    ...
    while (it != tools_.end()) {
        ...
        if (json.length() + tool_json.length() + 30 > kToolsListMaxPayloadSize) {
