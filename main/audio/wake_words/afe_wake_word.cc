...
bool AfeWakeWord::CreateResources() {
    ...
    // The SR low-cost pipeline cannot consume the continuous wake-word feed
    // fast enough on this board and eventually starves the rest of the system.
    // Keep wake detection in high-performance mode; the short-lived voice AFE
    // below remains low-cost to reduce the listening-time memory peak.
    ...
    if (afe_data_ == nullptr) {
        ESP_LOGE(TAG, "Failed to create wake word AFE");
        return false;
    }
    ...
    }, "audio_detection", 4096, this, 3, &audio_detection_task_,
       MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (task_result != pdPASS) {
        ESP_LOGE(TAG, "Failed to create audio detection task in PSRAM");
...
void AfeWakeWord::ReleaseResources() {
    ...
    if (audio_detection_task_ != nullptr) {
        vTaskDeleteWithCaps(audio_detection_task_);
...
void AfeWakeWord::Stop() {
    // fetch_with_delay() may be blocked inside the vendor AFE. Keep the task
    // and instance alive across conversations; forcing deletion can strand an
    // internal lock (especially on button-triggered listening).
    xEventGroupClearBits(event_group_, DETECTION_RUNNING_EVENT);
    if (afe_data_ != nullptr) {
...
void AfeWakeWord::AudioDetectionTask() {
    ...
    while (true) {
        ...
        // The history encoder runs on a different task. Do not mutate its PCM
        // deque when history upload is disabled; doing so wastes PSRAM and can
        // race with a stale encode task while wake detection is restarting.
#if CONFIG_SEND_WAKE_WORD_DATA
        StoreWakeWordData(res->data, res->data_size / sizeof(int16_t));
#endif

        if (res->wakeup_state == WAKENET_DETECTED) {
            // Stop feeding immediately, but let the application state handler
            // release this task and AFE from a different task context.
            xEventGroupClearBits(event_group_, DETECTION_RUNNING_EVENT);
